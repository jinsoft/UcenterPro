...
function check_db($dbhost, $dbuser, $dbpw, $dbname, $tablepre)
{
    ...
    if (!$connect = mysqli_connect($dbhost, $dbuser, $dbpw)) {
        ...
        if ($query = mysqli_query($connect, "SHOW TABLES FROM $dbname")) {
...
function createtable($sql, $link)
{
    ...
    return preg_replace("/^\s*(CREATE TABLE\s+.+\s+\(.+?\)).*$/isU", "\\1", $sql) .
        (mysqli_get_server_info($link) > '4.1' ? " ENGINE=$type DEFAULT CHARSET=" . DBCHARSET : " TYPE=$type");
}
...
function show_install()
{
    ...
    <div class="main">
        <div class="btnbox"><textarea name="notice" style="width: 80%;" readonly="readonly" id="notice"></textarea>
        </div>
        <div class="btnbox marginbot">
            <input type="button" name="submit" value="<?php echo lang('install_in_processed'); ?>" disabled
    ...
    foreach ($ret as $query) {
        ...
        if ($query) {
            ...
            if (substr($query, 0, 12) == 'CREATE TABLE') {
                ...
                $db->query(createtable($query, $link));
            } else {
                $db->query($query);
            }
