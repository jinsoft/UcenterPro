...
class template {
	...
	function complie() {
        ...
        $template = preg_replace_callback("/\{lang\s+(\w+?)\}/is", function($r) {
            return $this->lang($r[1]);
        },$template);
        ...
        $template = preg_replace_callback("/\<\?=(\@?\\\$[a-zA-Z_]\w*)((\[[\\$\[\]\w]+\])+)\?\>/is", function($r) {
            return $this->arrayindex($r[1], $r[2]);
        },$template);

        $template = preg_replace_callback("/\{\{eval (.*?)\}\}/is", function($r) {
            return $this->stripvtag("<? $r[1]?>");
        },$template);
        ...
        $template = preg_replace_callback("/\{for (.*?)\}/is", function($r) {
            return $this->stripvtag("<? for ($r[1]) {?>");
        },$template);
        ...
        $template = preg_replace_callback("/\{elseif\s+(.+?)\}/is", function($r) {
            return $this->stripvtag("<? } elseif($r[1]) { ?>");
        },$template);

        for($i=0; $i<2; $i++) {
            $template = preg_replace_callback("/\{loop\s+$this->vtag_regexp\s+$this->vtag_regexp\s+$this->vtag_regexp\}(.+?)\{\/loop\}/is", function($r) {
                return $this->loopsection($r[1], $r[2], $r[3], $r[4]);
            },$template);

            $template = preg_replace_callback("/\{loop\s+$this->vtag_regexp\s+$this->vtag_regexp\}(.+?)\{\/loop\}/is", function($r) {
                return $this->loopsection($r[1], '', $r[2], $r[3]);
            },$template);

        }

        $template = preg_replace_callback("/\{if\s+(.+?)\}/is", function($r) {
            return $this->stripvtag("<? if($r[1]) { ?>");
        },$template);

        $template = preg_replace("/\{template\s+(\w+?)\}/is", "<? include \$this->gettpl('\\1');?>", $template);

        $template = preg_replace_callback("/\{template\s+(.+?)\}/is", function($r) {
            return $this->stripvtag("<? include $this->gettpl($r[1]); ?>");
        },$template);
